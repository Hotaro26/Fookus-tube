...
object UpdateChecker {
    suspend fun check(context: Context, currentVersionName: String) {
        withContext(Dispatchers.IO) {
            try {
                val url = URL("https://api.github.com/repos/Hotaro26/fookus-tube/releases/latest")
                    ...
                    if (latestNum > currentNum) {
                        Handler(Looper.getMainLooper()).post {
                            AlertDialog.Builder(context)
                                ...
                                .setMessage("A new version of Fookus Tube (v\$latestVersion) is available! Would you like to download it?")
